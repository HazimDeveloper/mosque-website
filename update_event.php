...
$select_event = mysqli_query($connect,"SELECT * FROM user_event WHERE id_event = $id_event AND id_user = $id AND status = 'joined' ");
...
if($insert_event){
    header("location: event.php?id_masjid=$id_masjid");
}else{
    echo "sql error";
}
}else{
    echo "<script>alert('You already join this event!!')</script>"; 
    echo "<script>window.location = 'event.php?id_masjid=$id_masjid'</script>"; 
}
?>